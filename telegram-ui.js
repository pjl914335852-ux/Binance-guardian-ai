...
class TelegramUI {
  constructor(bot, config, state) {
    ...
    // 初始化语音处理器（如果配置了 OpenAI API Key）
    if (config.openai && config.openai.apiKey) {
      try {
        this.voiceHandler = new VoiceHandler(config.openai.apiKey);
        console.log('✅ 语音功能已启用');
      } catch (error) {
      ...
      console.log('⚠️ 未配置 OpenAI API Key，语音功能已禁用');
      this.voiceHandler = null;
    }
  ...
  setupCommands() {
    ...
    // Handle all non-command messages (text, voice, etc.)
    this.bot.on('message', (msg) => {
      ...
      if (msg.text) {
        // First try text input (for waiting states like password, pair input, etc.)
...
🎙️ *语音提问*${this.voiceHandler ? '（已启用）' : '（未启用）'}：
• 点击输入框旁的 🎤 按钮
• 说出你的问题
• 我会自动识别并回答
...
• 🛡️ Check coin safety
• 📊 Smart risk score (4-dimension assessment)
• 🎙️ Voice safety report (elderly-friendly)
• 🗣️ Voice chat (speak to ask)
• 📚 Learn safety knowledge
• 🚨 Emergency help (self-help guide)
• 📖 Real scam case education

*💡 Quick Start:*

💬 *Text Questions*:
• Enter coin name directly (e.g., "BTC")
• "Is XX coin safe?"
• "How to prevent scams?"

🎙️ *Voice Questions*${this.voiceHandler ? ' (Enabled)' : ' (Disabled)'}:
• Click 🎤 button next to input
• Speak your question
• I'll recognize and answer

*🎙️ Voice Report Usage:*

1. Click "🎙️ Voice Report" button
2. Select Chinese or English
3. Long-press to send to elderly
4. Daily updates with scam alerts and safety tips

💡 *Suitable for:*
• Elderly who prefer audio
    ...
    if (this.guardianMode) {
      ...
      keyboard = {
        inline_keyboard: [
          ...
          [
            { text: this.lang === 'zh' ? '🛡️ 快速验证（支持合约）' : '🛡️ Quick Check (Contract OK)', callback_data: 'check_coin' },
            { text: this.lang === 'zh' ? '📊 投资分析（仅币种）' : '📊 Investment Analysis (Coin Only)', callback_data: 'risk_score' }
          ],
          // 第二组：学习功能（2个按钮）
          [
          ...
          // 第三组：语音和求助（2个按钮）
          [
            { text: this.lang === 'zh' ? '🎙️ 语音播报' : '🎙️ Voice Report', callback_data: 'voice_report' },
            { text: this.lang === 'zh' ? '🚨 紧急求助' : '🚨 Emergency', callback_data: 'emergency_help' }
          ],
          // 分隔线（空行效果）
          // 第四组：设置（2个按钮）
          [
      ...
      keyboard = {
        inline_keyboard: [
          ...
          // 第七组：推送和系统（2个按钮）
          [
            { text: this.lang === 'zh' ? '🔔 推送开关' : '🔔 Push Toggle', callback_data: 'toggle_push' },
...
🛡️ Making crypto investment safer
    ...
    const keyboard = {
      inline_keyboard: [
        [
          { text: this.lang === 'zh' ? '🎙️ 语音功能设置' : '🎙️ Voice Setup', callback_data: 'voice_setup' }
        ],
        [
          { text: this.lang === 'zh' ? '🏠 返回主菜单' : '🏠 Back to Menu', callback_data: 'start' }
        ]
  ...
  // Handle voice setup guide
  handleVoiceSetup(chatId) {
    // 检查当前状态
    const isEnabled = !!this.voiceHandler;
    const hasApiKey = !!(this.config.openai && this.config.openai.apiKey);
    
    const statusText = this.lang === 'zh' ? 
      (isEnabled ? '✅ 已启用' : '❌ 未启用') :
      (isEnabled ? '✅ Enabled' : '❌ Disabled');
    
    const apiKeyText = hasApiKey ? 
      (this.lang === 'zh' ? `*API Key：* ${this.config.openai.apiKey.substring(0, 20)}...` : `*API Key:* ${this.config.openai.apiKey.substring(0, 20)}...`) :
      (this.lang === 'zh' ? '*API Key：* 未配置' : '*API Key:* Not configured');
...
点击下方"配置 API Key"按钮，然后：
1. 发送你的 OpenAI API Key
2. Bot 自动保存并重启
3. 立即可用！
...
Click "Configure API Key" button below, then:
1. Send your OpenAI API Key
2. Bot saves and restarts automatically
3. Ready to use!

*💡 How to Get API Key:*

1. Visit: platform.openai.com/api-keys
2. Sign in and create new key
3. Copy key (sk-proj-xxx...)
4. Send to me

*💰 Cost:* ~¥0.1 per conversation
*🔒 Security:* Encrypted storage, local use only
    `.trim();
    
    const keyboard = {
      inline_keyboard: [
        [
          { text: this.lang === 'zh' ? '🔧 配置 API Key' : '🔧 Configure API Key', callback_data: 'voice_config_api' }
        ],
        [
          { text: this.lang === 'zh' ? '📖 详细教程' : '📖 Full Guide', callback_data: 'voice_full_guide' },
          { text: this.lang === 'zh' ? '🧪 测试功能' : '🧪 Test', callback_data: 'voice_test' }
        ],
        [
          { text: this.lang === 'zh' ? '🔙 返回帮助' : '🔙 Back to Help', callback_data: 'help' },
          { text: this.lang === 'zh' ? '🏠 主菜单' : '🏠 Menu', callback_data: 'start' }
        ]
      ]
    };
    ...
    }).catch(err => console.error('发送语音设置指南失败:', err));
  }
  
  // Handle voice config API key
  handleVoiceConfigApi(chatId, messageId, queryId) {
...
3. Copy full key
4. Send to me directly

*⚠️ Note:*
• Key starts with \`sk-proj-\`
• Length ~100+ characters
• Auto-save and restart after sending
...
Send /cancel to cancel
    ...
    const keyboard = {
      inline_keyboard: [
        [
          { text: this.lang === 'zh' ? '❌ 取消' : '❌ Cancel', callback_data: 'voice_setup' }
        ]
      ]
    };
    ...
    setTimeout(() => {
      this.bot.sendMessage(chatId, text, {
        ...
        reply_markup: keyboard,
        disable_web_page_preview: true
      }).catch(err => console.error('发送配置提示失败:', err));
    }, 200);
    
    // 设置等待状态
    this.userStates[chatId] = { action: 'waitingForOpenAIKey' };
  }
...
Enter coin name or paste contract address:
      `.trim();
      ...
      this.bot.answerCallbackQuery(query.id);
      this.bot.deleteMessage(chatId, messageId).catch(() => {});
      this.handleRiskScore({ chat: { id: chatId } });
      
    } else if (data === 'voice_report') {
      // Voice safety report
      this.bot.answerCallbackQuery(query.id);
      this.bot.deleteMessage(chatId, messageId).catch(() => {});
      this.handleVoiceReport({ chat: { id: chatId } });
    ...
    } else if (data === 'scam_case') {
      // Today's scam case
      this.bot.answerCallbackQuery(query.id);
      this.bot.deleteMessage(chatId, messageId).catch(() => {});
      this.handleScamCase({ chat: { id: chatId } });
...
Click buttons below to select a topic 👇
      ...
      this.handleEmergencyType(chatId, messageId, query.id, type);
    } else if (data === 'voice_zh' || data === 'voice_en') {
      // Voice report language selection
      this.bot.answerCallbackQuery(query.id);
      this.bot.deleteMessage(chatId, messageId).catch(() => {});
      const lang = data === 'voice_zh' ? 'zh' : 'en';
      this.handleVoiceReportLang(chatId, lang);
    } else {
      // Unknown callback - answer to stop loading animation
      console.log('Unknown callback_data:', data);
      this.bot.answerCallbackQuery(query.id, {
        text: this.lang === 'zh' ? '⚠️ 功能暂未实现' : '⚠️ Feature not implemented',
        show_alert: false
...
Wait for next auto-push or check market overview.
  ...
  async handleTextInput(msg) {
    ...
    // Handle user states (OpenAI API Key, etc.)
    if (this.userStates && this.userStates[chatId]) {
      const state = this.userStates[chatId];
      
      if (state.action === 'waitingForOpenAIKey') {
        delete this.userStates[chatId];
        
        // Validate API key format
        if (!text.startsWith('sk-proj-') && !text.startsWith('sk-')) {
          this.bot.sendMessage(chatId, this.lang === 'zh' ?
            '❌ API Key 格式错误！\n\n正确格式应该以 `sk-proj-` 或 `sk-` 开头\n\n请重新发送或点击 /cancel 取消' :
            '❌ Invalid API Key format!\n\nShould start with `sk-proj-` or `sk-`\n\nPlease resend or /cancel',
            { parse_mode: 'Markdown' }
          );
          this.userStates[chatId] = state; // 恢复状态
          return;
        }
        ...
        this.config.openai.apiKey = text;
...
Please wait 5-10 seconds, then:
1. Send voice message to test
2. Or click "Voice Setup" to check status

💡 Voice recognition will be enabled after restart
        `.trim();
...
💡 You can view and manage all alerts in "Price Alerts" menu
    ...
    if (this.waitingForRiskScore === chatId) {
      ...
      if (/^0x[a-fA-F0-9]{40}$/.test(text)) {
        this.bot.sendMessage(chatId, this.lang === 'zh' ? 
          '⚠️ *投资分析不支持合约地址*\n\n💡 合约地址请使用 *"🛡️ 快速验证"*\n\n或输入币种名称继续' :
          '⚠️ *Investment analysis does not support contract*\n\n💡 Use *"🛡️ Quick Check"* for contract\n\nOr enter coin name to continue'
        , { parse_mode: 'Markdown' });
        return;
      }
...
📊 *Risk Levels:*
• 🟢 Safe (80-100) → Suitable
• 🟡 Low Risk (60-79) → Caution
• 🟠 Medium Risk (40-59) → Small amount
...
Select language:
    ...
    const keyboard = {
      inline_keyboard: [
        [
          { text: this.lang === 'zh' ? '🇨🇳 中文播报' : '🇨🇳 Chinese', callback_data: 'voice_zh' },
          { text: this.lang === 'zh' ? '🇬🇧 英文播报' : '🇬🇧 English', callback_data: 'voice_en' }
        ],
        [
          { text: this.lang === 'zh' ? '🔙 返回主菜单' : '🔙 Back to Menu', callback_data: 'start' }
        ]
  ...
  // Handle voice report in specific language
  handleVoiceReportLang(chatId, lang) {
    this.bot.sendMessage(chatId, this.lang === 'zh' ? 
      '🎙️ 正在生成安全播报...' :
      '🎙️ Generating safety report...'
    );
    ...
    try {
      ...
      // 生成指定语言的播报
      const scriptPath = path.join(__dirname, 'voice-safety-report.js');
      execSync(`node ${scriptPath} ${lang}`, { encoding: 'utf8' });
      
      // 读取生成的文本
      const today = new Date().toISOString().split('T')[0];
      const reportPath = path.join(__dirname, 'reports', `safety-report-${today}-${lang}.txt`);
      
      if (fs.existsSync(reportPath)) {
        const reportText = fs.readFileSync(reportPath, 'utf8');
        const title = lang === 'zh' ? '🎙️ *今日安全播报（中文）*' : '🎙️ *Today\'s Safety Report (English)*';
        
        const instructions = lang === 'zh' ? `

📱 *如何使用：*

1️⃣ *iPhone*: 长按文字 → 选择"朗读"
2️⃣ *Android*: 长按文字 → 选择"朗读所选内容"
3️⃣ *分享*: 长按消息 → 转发给家人

💡 每天内容不同，包含最新骗局预警和安全提示！
        `.trim() : `

📱 *How to Use:*

1️⃣ *iPhone*: Long-press text → Select "Speak"
2️⃣ *Android*: Long-press text → Select "Read Aloud"
3️⃣ *Share*: Long-press message → Forward to family

💡 Daily content changes with latest scam alerts and safety tips!
        `.trim();
        
        const keyboard = {
          inline_keyboard: [
            [
              { text: this.lang === 'zh' ? '🔄 切换语言' : '🔄 Switch Language', callback_data: 'voice_report' },
              { text: this.lang === 'zh' ? '🔙 返回' : '🔙 Back', callback_data: 'start' }
            ]
          ]
        };
...
💡 Community experts will respond within 5 minutes!
  ...
  handleScamCase(msg) {
    ...
    try {
      ...
      // 获取今日案例
      const scriptPath = path.join(__dirname, 'scam-cases.js');
      const lang = this.lang === 'zh' ? 'zh' : 'en';
      const output = execSync(`node ${scriptPath} today ${lang}`, { encoding: 'utf8' });
  ...
  async handleEmergencyType(chatId, messageId, queryId, type) {
    this.bot.answerCallbackQuery(queryId);
    this.bot.deleteMessage(chatId, messageId).catch(() => {});
    
    const guides = {
      'scam': {
        title: this.lang === 'zh' ? '疑似骗局 - 自救指南' : 'Suspected Scam - Self-Help Guide',
...
点击下方按钮查看更多防骗知识
        `.trim() : `
🚨 *Suspected Scam - Self-Help Guide*

*⚠️ Emergency Actions (Do Now):*

1️⃣ *Stop All Operations*
...
• *Fake Support*: Contacts you first, asks for codes
• *Fake Airdrop*: Requires wallet connection and approval
• *Fake Exchange*: Similar domain but not official
• *High Returns*: Promises 5-10% daily returns
• *Fake Coin*: Claims mainnet launch soon
...
Click buttons below for more anti-scam knowledge
        `.trim()
      },
      'transaction': {
        title: this.lang === 'zh' ? '交易问题 - 自助排查' : 'Transaction Issue - Self-Check',
        content: this.lang === 'zh' ? `
💼 *交易问题 - 自助排查*
...
• 提供交易哈希（TxID）可加快处理
        `.trim() : `
💼 *Transaction Issue - Self-Check*

*🔍 Common Issues:*

1️⃣ *Deposit Not Arrived*
   • Wait time: 10-30 minutes
   • Check address correctness
   • View blockchain explorer
   • Confirm network (ERC20/BEP20)

2️⃣ *Withdrawal Failed*
   • Check address format
   • Confirm network match
   • Check minimum withdrawal
   • Verify identity completion

3️⃣ *Transaction Stuck*
   • Refresh page
   • Clear cache
...
• Binance Deposit: Wallet → Deposit History
• Binance Withdrawal: Wallet → Withdrawal History
...
• 90% issues resolve within 30 minutes
• Contact Binance support if > 1 hour
• Provide TxID for faster processing
        `.trim()
      },
      'account': {
        title: this.lang === 'zh' ? '账户安全 - 紧急处理' : 'Account Security - Emergency',
        content: this.lang === 'zh' ? `
🔐 *账户安全 - 紧急处理*
...
• 不要分享验证码
        `.trim() : `
🔐 *Account Security - Emergency*

*⚠️ Account Anomaly Actions:*
...
• Binance Support: support@binance.com
• Live Chat: Binance App → Support
• Phone: Check Binance official site
...
• Don't share verification codes
      ...
      'technical': {
        title: this.lang === 'zh' ? '技术问题 - 快速解决' : 'Technical Issue - Quick Fix',
        content: this.lang === 'zh' ? `
💻 *技术问题 - 快速解决*
...
2️⃣ *Slow Page Loading*
   • Refresh page (F5)
   • Clear cache
   • Check network speed
   • Try mobile app

3️⃣ *No Verification Code*
   • Check spam folder
   • Wait 1-2 minutes
   • Resend code
...
4. If still issues, contact support
      ...
      'other': {
        title: this.lang === 'zh' ? '其他问题 - 帮助中心' : 'Other Issues - Help Center',
        content: this.lang === 'zh' ? `
❓ *其他问题 - 帮助中心*
...
1️⃣ *币安学院*
   • 网址：academy.binance.com
   • 内容：基础知识、交易教程
   • 适合：新手学习

2️⃣ *帮助中心*
   • 网址：binance.com/support
   • 内容：常见问题解答
   • 搜索：输入关键词

3️⃣ *社区论坛*
   • 网址：binance.com/community
   • 内容：用户讨论、经验分享
   • 互助：提问和回答
...
• 在线客服：币安 App → 客服
• 邮件：support@binance.com
• 社交媒体：
  - Twitter: @binance
  - Telegram: @binance_chinese
...
• 价格提醒：设置价格提醒
        `.trim() : `
❓ *Other Issues - Help Center*

*📚 Self-Help Resources:*

1️⃣ *Binance Academy*
   • URL: academy.binance.com
   • Content: Basics, trading tutorials
   • For: Beginners

2️⃣ *Help Center*
   • URL: binance.com/support
   • Content: FAQ
   • Search: Enter keywords

3️⃣ *Community Forum*
   • URL: binance.com/community
   • Content: User discussions, experiences
   • Help: Ask and answer
...
• Trading fee: 0.1% (lower for VIP)
• Withdrawal fee: Varies by coin
• Deposit fee: Free (network fee excluded)
• Check rates: binance.com/fee

*🔍 Common Feature Questions:*

• How to deposit? App → Wallet → Deposit
• How to withdraw? App → Wallet → Withdraw
• How to trade? App → Trade → Spot
• How to check orders? App → Orders → History
...
• Check authorized devices regularly

*💡 Asking Tips:*

• Describe problem clearly
• Provide screenshots (hide sensitive info)
• Mention tried solutions
• Provide account info (UID)

*🎯 This Bot Features:*

• Check coin safety: Enter coin name
• Learn safety: Click "Today's Lesson"
• View scam cases: Click "Today's Case"
• Voice recognition: Send voice message
    ...
    if (!guide) {
      this.bot.sendMessage(chatId, this.lang === 'zh' ? 
        ...
        '❌ Unknown help type',
        {
          reply_markup: {
            inline_keyboard: [[
              { text: this.lang === 'zh' ? '🔙 返回' : '🔙 Back', callback_data: 'emergency_help' }
            ]]
          }
        }
      );
...
Sorry, voice recognition is not enabled.
...
Guide: Click "Voice Setup" button below
      `.trim();
      ...
      this.bot.sendMessage(chatId, text, {
        parse_mode: 'Markdown',
        reply_markup: keyboard
      }).catch(err => console.error('发送语音未启用消息失败:', err));
      return;
    }
    ...
    let processingMsg;
    try {
      processingMsg = await this.bot.sendMessage(chatId, this.lang === 'zh' ? 
        '🎙️ 正在识别语音...' :
        '🎙️ Recognizing voice...'
      );
    } catch (err) {
      console.error('发送处理中消息失败:', err);
      return;
    }
    ...
    try {
      ...
      const path = require('path');
      const tempPath = path.join(this.voiceHandler.tempDir, `voice_${Date.now()}.ogg`);
      
      await this.voiceHandler.downloadFile(fileUrl, tempPath);
      
      // 3. 语音转文字
      const text = await this.voiceHandler.speechToText(tempPath, this.lang);
      
      // 4. 清理临时文件
      this.voiceHandler.cleanupFile(tempPath);
      
      // 5. 删除处理中提示（添加延迟避免 409）
      setTimeout(() => {
        this.bot.deleteMessage(chatId, processingMsg.message_id).catch(() => {});
      }, 100);
      
      // 6. 显示识别结果
      const resultText = this.lang === 'zh' ? 
        `🎙️ *识别结果：*\n\n"${text}"\n\n正在为你查询...` :
        `🎙️ *Recognized:*\n\n"${text}"\n\nSearching for you...`;
      
      await this.bot.sendMessage(chatId, resultText, { parse_mode: 'Markdown' });
      
      // 7. 当作文字消息处理（添加延迟避免 409）
      setTimeout(() => {
        const textMsg = { ...msg, text: text };
        this.handleSmartMessage(textMsg);
      }, 200);
...
💡 Suggestion: Use text input or try again later`;
