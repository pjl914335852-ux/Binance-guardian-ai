...
// ==================== Code Protection ====================
// Simple integrity check - prevents casual modifications
...
function verifyIntegrity() {
  ...
  const authorizedEnv = process.env.GUARDIAN_AUTHORIZED === 'true' || 
                        process.env.NODE_ENV === 'development' ||
                        !process.env.GUARDIAN_AUTHORIZED; // Allow if not set (backward compatibility)
  
  if (!authorizedEnv) {
    console.error('❌ Unauthorized environment detected');
    console.error('This software is protected by proprietary license.');
    console.error('For licensing inquiries: https://github.com/pjl914335852-ux/Binance-guardian-ai');
    process.exit(1);
  }
}
