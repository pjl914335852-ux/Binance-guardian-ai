...
- **9 条区块链支持**: 
  ...
  - 非 EVM 链: Solana
- **智能链识别**: 根据用户选择或上下文自动识别网络
- **链选择界面**: 用户友好的按钮选择界面
- **免费 API**: 使用公开的区块链浏览器 API，无需 API key

**🛡️ 合约安全评分系统**
- **0-100 分评分**: 直观的数字评分，普通用户一看就懂
- **4 个风险等级**: 
  - 🟢 80-100分: 低风险
  - 🟡 60-79分: 中等风险
  - 🟠 40-59分: 高风险
...
**📌 免责声明**
- **数据延迟提醒**: 检测结果仅供参考，可能存在延迟
- **多次验证建议**: 链上数据实时变化，建议多次验证
- **风险提示**: 投资有风险，决策需谨慎

#### 修复问题

**🐛 链选择功能修复**
- 修复所有链都显示为 "Ethereum" 的问题
- 添加完整的网络名称映射表
- 支持所有 9 条链的正确显示

**🐛 评分信息传递修复**
- 修复评分信息在传递过程中丢失的问题
- 确保 score、riskLevelText、riskEmoji 正确传递

**🐛 界面重复问题修复**
- 修复合约检测后返回主菜单出现两个界面的问题
- 优化消息删除顺序，避免消息 ID 混乱
